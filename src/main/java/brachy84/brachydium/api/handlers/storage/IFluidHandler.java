...
public interface IFluidHandler extends Clearable, InventoryListener, INotifiableHandler {
    ...
    default void setFluid(int slot, FluidStack stack) {
        setFluidSilently(slot, stack);
        markDirty();
    }

    /**
     * Sets the fluid in the slot without causing listeners to run.
     * Good for initialisation or when {@link #setFluid(int, FluidStack)} is called a lot
     *
     * @param slot  to set
     * @param stack to set
     */
    void setFluidSilently(int slot, FluidStack stack);
    ...
    default void clear() {
        for (int i = 0; i < size(); i++) {
            setFluidSilently(i, FluidStack.EMPTY);
        }
        markDirty();
    }
}
