...
public interface IToolItem {
     ...
     */
    default int damageItem(ItemStack stack, int damage, boolean allowPartial, boolean simulate) {
        int maxDmg = getMaxItemDamage(stack);
        int dmg = getItemDamage(stack);
        if (maxDmg - dmg < damage && !allowPartial)
            return 0;
        damage = Math.min(damage, maxDmg - dmg);
        if (!simulate) {
            setDamage(stack, dmg + damage);
        }
        if (damage + dmg >= maxDmg) {
            stack.setCount(0);
        }
        return damage;
    }
